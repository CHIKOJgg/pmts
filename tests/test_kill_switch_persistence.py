...
@pytest.fixture
def kill_switch():
    return KillSwitch(confirmation_token="test-token")
...
def test_kill_switch_persistence_reset(risk_engine, store):
    ...
    # 2. Reset
    success = risk_engine.reset_kill_switch(confirmation_token="test-token")
    assert success
    assert not risk_engine.kill_switch_active
    assert not store.load_kill_switch()
...
def test_kill_switch_recovery_on_startup(db_path, portfolio):
    ...
    ks1 = KillSwitch(confirmation_token="test-token")
    ...
    ks2 = KillSwitch(confirmation_token="test-token")
