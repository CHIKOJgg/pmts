...
def make_risk(portfolio=None, limits=None, kill_token="test-token"):
...
def make_proposal(
    size=50.0, price=0.50, side=Side.BUY_YES,
    strategy=StrategyId.MM, market="BTC-Q4",
    platform=Platform.POLYMARKET,
) -> OrderProposal:
    return OrderProposal(
        proposal_id=uid(), market_id=market, platform=platform,
        side=side, size_usdc=size, limit_price=price,
        order_type=OrderType.LIMIT, strategy_id=strategy,
        expiry_ms=now_ms() + 30_000, source_ts=now_ms(),
    )
...
def make_submission(
    ...
    return OrderSubmission(
        order_id=uid(), proposal_id=uid(),
        market_id="BTC-Q4", platform=Platform.POLYMARKET,
        side=side, size_usdc=size, limit_price=price,
        order_type=OrderType.LIMIT, strategy_id=strategy,
...
def make_snapshot(
    market="BTC-Q4", platform=Platform.POLYMARKET,
    yes_bid=0.44, yes_ask=0.46,
    bid_depth=1000.0, ask_depth=900.0, fee_bps=20,
    is_stale=False,
) -> MarketSnapshot:
    t = now_ms()
    no_mid = 1.0 - (yes_bid + yes_ask) / 2
    return MarketSnapshot(
        market_id=market, platform=platform,
        yes_bid=yes_bid, yes_ask=yes_ask,
        no_bid=max(0.01, no_mid - 0.01),
        no_ask=min(0.99, no_mid + 0.01),
        bid_depth_usdc=bid_depth, ask_depth_usdc=ask_depth,
        taker_fee_bps=fee_bps, ts=t, received_ts=t, is_stale=is_stale,
    )

def make_fv(
    market="BTC-Q4", mid_pm=0.46, mid_op=0.54,
    spread_pm=0.02, spread_op=0.02,
    ofi_pm=0.0, ofi_op=0.0, vol_30s=0.01,
    days=10.0, delta=0.0,
    bid_pm=1000.0, ask_pm=900.0, bid_op=800.0, ask_op=700.0,
    stale=None,
) -> FeatureVector:
    stale = stale or []
    t = now_ms()
    if stale:
        ...
        arb = (
            1.0
            - (mid_pm + spread_pm / 2)
            - ((1 - mid_op) + spread_op / 2)
            - 0.002 - 0.0025
        )
    return FeatureVector(
        market_id=market, ts=t, computed_ts=t,
        arb_signal=arb, stale_markets=stale,
        mid_pm=mid_pm, mid_op=mid_op,
        spread_pm=spread_pm, spread_op=spread_op,
        ofi_pm=ofi_pm, ofi_op=ofi_op,
        vol_30s=vol_30s, days_to_resolution=days,
        portfolio_delta=delta,
        bid_depth_pm=bid_pm, ask_depth_pm=ask_pm,
        bid_depth_op=bid_op, ask_depth_op=ask_op,
    )
...
class TestTypes(unittest.TestCase):
    ...
    def test_market_snapshot_validation(self):
        with self.assertRaises(CrossedBookError):
            MarketSnapshot(
                ...
                yes_bid=0.50, yes_ask=0.40,   # bid > ask → crossed
                no_bid=0.40, no_ask=0.60,
    ...
    def test_feature_vector_nan_requires_stale(self):
        with self.assertRaises(ValueError):
            FeatureVector(
                market_id="X", ts=now_ms(), computed_ts=now_ms(),
                arb_signal=float("nan"), stale_markets=[],   # NaN but no stale
                mid_pm=0.5, mid_op=0.5, spread_pm=0.0, spread_op=0.0,
                ofi_pm=0.0, ofi_op=0.0, vol_30s=0.01, days_to_resolution=10.0,
                portfolio_delta=0.0, bid_depth_pm=0.0, ask_depth_pm=0.0,
                bid_depth_op=0.0, ask_depth_op=0.0,
            )
    ...
    def test_order_proposal_arb_requires_group(self):
        with self.assertRaises(ValueError):
            OrderProposal(
                proposal_id=uid(), market_id="X", platform=Platform.POLYMARKET,
                ...
                expiry_ms=now_ms() + 1000, source_ts=now_ms(),
            )

    def test_execution_result_fill_price_required(self):
        with self.assertRaises(ValueError):
            ExecutionResult(
                proposal_id=uid(), exchange_order_id="x",
...
class TestOrderTracker(unittest.TestCase):
    ...
    def test_dust_tolerance(self):
        ...
        self.assertEqual(result.status, OrderStatus.FILLED,
                         "99.95/100 = 0.9995 ≥ threshold → should be FILLED")
    ...
    def test_expiry(self):
        sub = make_submission()
        sub = OrderSubmission(
            order_id=sub.order_id, proposal_id=sub.proposal_id,
            market_id=sub.market_id, platform=sub.platform,
            side=sub.side, size_usdc=sub.size_usdc, limit_price=sub.limit_price,
            order_type=sub.order_type, strategy_id=sub.strategy_id,
...
class TestPortfolioManager(unittest.TestCase):
    ...
    def test_record_fill_decreases_cash(self):
        ...
        run_async(pm.start())
        fill = FillRecord(
            proposal_id=uid(), order_id=uid(),
            market_id="X", platform=Platform.POLYMARKET,
            side="buy_yes", filled_usdc=100.0, fill_price=0.50, ts=now_ms(),
        )
    ...
    def test_get_delta_after_buy(self):
        pm = make_portfolio()
        run_async(pm.start())
        run_async(pm.record_fill(FillRecord(
            proposal_id=uid(), order_id=uid(),
            market_id="BTC", platform=Platform.POLYMARKET,
            side="buy_yes", filled_usdc=100.0, fill_price=0.50, ts=now_ms(),
        )))
    ...
    def test_delta_neutral_after_yes_and_no(self):
        ...
        for side in ["buy_yes", "buy_no"]:
            run_async(pm.record_fill(FillRecord(
                proposal_id=uid(), order_id=uid(),
                market_id="BTC", platform=Platform.POLYMARKET,
                side=side, filled_usdc=100.0, fill_price=0.50, ts=now_ms(),
            )))
        delta = pm.get_delta("BTC")
    ...
    def test_negative_holdings_raises(self):
        ...
        run_async(pm.record_fill(FillRecord(
            proposal_id=uid(), order_id=uid(),
            market_id="BTC", platform=Platform.POLYMARKET,
            side="buy_yes", filled_usdc=50.0, fill_price=0.50, ts=now_ms(),
        )))
        with self.assertRaises(NegativeHoldings):
            run_async(pm.record_fill(FillRecord(
                proposal_id=uid(), order_id=uid(),
                market_id="BTC", platform=Platform.POLYMARKET,
                side="sell_yes", filled_usdc=200.0, fill_price=0.55, ts=now_ms(),
            )))
        run_async(pm.stop())
...
class TestKillSwitch(unittest.TestCase):
    ...
    def test_correct_token_resets(self):
        ...
        self.assertTrue(ks.reset("abc", "ops"))
        self.assertFalse(ks.is_active)
...
class TestRiskEngine(unittest.TestCase):
    ...
    def test_toctou_fix_second_proposal_sees_reservation(self):
        ...
        self.assertIn(d2.reject_reason, (
    ...
    def test_terminal_release(self):
        ...
        self.assertTrue(d2.approved, f"After terminal, capacity should be restored. Got {d2.reject_reason}: {d2.reject_detail}")
        run_async(pm.stop())
    ...
    def test_projected_delta_check(self):
        ...
        run_async(pm.record_fill(FillRecord(
            proposal_id=uid(), order_id=uid(),
            market_id="BTC", platform=Platform.POLYMARKET,
            side="buy_yes", filled_usdc=40.0, fill_price=0.50, ts=now_ms(),
        )))
        risk, _, _ = make_risk(pm, lim)
    ...
    def test_buy_no_reduces_delta_allowed(self):
        ...
        run_async(pm.record_fill(FillRecord(
            proposal_id=uid(), order_id=uid(),
            market_id="BTC", platform=Platform.POLYMARKET,
            side="buy_yes", filled_usdc=20.0, fill_price=0.50, ts=now_ms(),
        )))
        # delta = +40 tokens
        risk, _, _ = make_risk(pm, lim)
        # Buy NO: 10/0.50 = 20 tokens → projected = 40-20 = 20 < 50 ✓
        d = risk.evaluate(make_proposal(
            size=10.0, price=0.50, side=Side.BUY_NO, market="BTC"
        ))
...
class TestSignalContext(unittest.TestCase):
    ...
    def test_valid_construction(self):
        ctx = SignalContext(
            market_id="X", confidence_multiplier=1.2,
            regime=MarketRegime.STABLE, vol_regime=VolRegime.NORMAL,
            suppress_mm=False, arb_quality=0.8, hedge_urgency=0.1,
            model_version="v1", inference_ms=5.0, feature_count=20,
            is_fallback=False,
        )
    ...
    def test_confidence_out_of_range(self):
        with self.assertRaises(ValueError):
            SignalContext(
                market_id="X", confidence_multiplier=0.0,
                regime=MarketRegime.UNKNOWN, vol_regime=VolRegime.NORMAL,
                suppress_mm=False, arb_quality=0.5, hedge_urgency=0.0,
                model_version="v", inference_ms=0, feature_count=0, is_fallback=True,
            )
    ...
    def test_total_blackout_prevented(self):
        ...
        with self.assertRaises(ValueError):
            SignalContext(
                ...
                regime=MarketRegime.THIN, vol_regime=VolRegime.SPIKE,
                suppress_mm=True, arb_quality=0.0, hedge_urgency=0.0,
                model_version="v", inference_ms=0, feature_count=0, is_fallback=True,
            )

    def test_is_urgent_hedge(self):
        ctx = SignalContext(
            market_id="X", confidence_multiplier=1.0,
            regime=MarketRegime.UNKNOWN, vol_regime=VolRegime.NORMAL,
            suppress_mm=False, arb_quality=0.5, hedge_urgency=0.85,
            model_version="v", inference_ms=0, feature_count=0, is_fallback=True,
        )
...
class TestArbitrageStrategy(unittest.TestCase):
    ...
    def test_profitable_arb_accepted(self):
        ...
        fv     = make_fv(
            mid_pm=0.35, mid_op=0.72,
            spread_pm=0.01, spread_op=0.01,
            ask_pm=2000.0, ask_op=1500.0,
        )
        result = strat.evaluate(fv)
    ...
    def test_arb_cross_venue(self):
        strat  = self._strat(min_net_edge=0.001, max_signal_age_ms=60_000)
        fv     = make_fv(mid_pm=0.35, mid_op=0.72,
                         ask_pm=2000.0, ask_op=1500.0)
        result = strat.evaluate(fv)
        if result.accepted:
            self.assertNotEqual(
...
class TestFillSimulator(unittest.TestCase):
    ...
    def test_fill_when_crossing(self):
        ...
        if fills:
            self.assertAlmostEqual(fills[0].fill_price, 0.46)
    ...
    def test_expiry_cancels_order(self):
        ...
        sub = OrderSubmission(
            order_id=uid(), proposal_id=uid(),
            market_id="BTC-Q4", platform=Platform.POLYMARKET,
            side=Side.BUY_YES, size_usdc=50.0, limit_price=0.30,
        ...
        p = OrderProposal(
            proposal_id=sub.proposal_id, market_id=sub.market_id,
            platform=sub.platform, side=sub.side,
            size_usdc=sub.size_usdc, limit_price=sub.limit_price,
            order_type=sub.order_type, strategy_id=sub.strategy_id,
            expiry_ms=sub.expiry_ms, source_ts=t,
        )
        self.sim.submit(p, t)
    ...
    def test_slippage_computed(self):
        ...
        fills = [e for e in events if e.filled_usdc > 0]
        if fills:
            # fill_price=0.46, limit=0.50 → |0.46-0.50|/0.50 * 10000 ≈ 800bps
            # Allow 799 or 800 due to float representation (0.04/0.50 = 799.999...)
            self.assertIsNotNone(fills[0].slippage_bps)
            self.assertIn(fills[0].slippage_bps, (799, 800),
                         f"Expected ~800 bps, got {fills[0].slippage_bps}")
...
class TestBacktestEngine(unittest.TestCase):
    ...
    def test_multi_market(self):
        ...
        engine  = BacktestEngine(
            ...
            initial_capital=5_000.0, seed=3,
        )
