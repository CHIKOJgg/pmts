...
def heuristic_enhance(fv: FeatureVector) -> SignalContext:
    ...
    if math.isnan(fv.arb_signal) or fv.arb_signal <= 0:
        ...
        arb_quality = max(0.0, min(1.0,
            base * spr_penalty * depth_score * ofi_score * regime_f
        ))
    ...
    if ((fv.ofi_pm > 0.2 and fv.ofi_op < -0.2) or
            (fv.ofi_pm < -0.2 and fv.ofi_op > 0.2)):
        conf *= 0.80
    ...
    # ── MM suppression ────────────────────────────────────────────────────────
    suppress_mm = (
        (days is not None and days <= _DAYS_NO_MM)
        or vol_regime == VolRegime.SPIKE
        or regime == MarketRegime.THIN
    )

    # Safety: cannot suppress both arb and MM simultaneously
    if suppress_mm and arb_quality < 0.01 and conf <= CONFIDENCE_MIN:
        arb_quality = 0.05
    ...
    vol_amp = {
        VolRegime.LOW: 0.7, VolRegime.NORMAL: 1.0,
        VolRegime.HIGH: 1.3, VolRegime.SPIKE: 1.6,
    }.get(vol_regime, 1.0)
