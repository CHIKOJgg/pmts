...
class ArbitrageStrategy:
    ...
    def __init__(
        ...
        self._cfg       = config
        self._pm_fee    = pm_fee_bps
        self._op_fee    = op_fee_bps
    ...
    def evaluate(self, fv: FeatureVector, now_ts: Optional[int] = None) -> ArbEvaluation:
        ...
        def _reject(reason: str) -> ArbEvaluation:
            return ArbEvaluation(
                ...
                signal_age_ms=signal_age_ms, arb_signal=fv.arb_signal,
        ...
        if signal_age_ms > self._cfg.max_signal_age_ms:
            self.rejected_stale += 1
            return _reject(
                f"signal_age={signal_age_ms}ms > {self._cfg.max_signal_age_ms}ms"
            )
        ...
        if net_a >= net_b:
            ...
            l1_plat, l1_side, l1_ask, l1_bid = (
                Platform.POLYMARKET, Side.BUY_YES, yes_ask_pm,
                fv.mid_pm - fv.spread_pm / 2,
            )
            l2_plat, l2_side, l2_ask, l2_bid = (
                Platform.OPINION, Side.BUY_NO, no_ask_op,
            ...
            l1_plat, l1_side, l1_ask, l1_bid = (
                Platform.OPINION, Side.BUY_YES, yes_ask_op,
                fv.mid_op - fv.spread_op / 2,
            )
            l2_plat, l2_side, l2_ask, l2_bid = (
                Platform.POLYMARKET, Side.BUY_NO, no_ask_pm,
        ...
        if raw_size < self._cfg.min_order_usdc:
            ...
            return ArbEvaluation(
                market_id=fv.market_id, evaluated_at=now,
                signal_age_ms=signal_age_ms, arb_signal=fv.arb_signal,
                accepted=False, direction=direction, raw_size_usdc=raw_size,
                rejection_reason=(
        ...
        if net_edge < self._cfg.min_net_edge:
            ...
            return ArbEvaluation(
                market_id=fv.market_id, evaluated_at=now,
                signal_age_ms=signal_age_ms, arb_signal=fv.arb_signal,
                accepted=False, direction=direction,
        ...
        try:
            leg1 = OrderProposal(
                ...
                market_id=fv.market_id, platform=l1_plat, side=l1_side,
                size_usdc=round(final_size, 2),
                limit_price=round(min(0.999, l1_ask), 4),
                ...
                expiry_ms=expiry_ms, source_ts=fv.ts,
            )
            leg2 = OrderProposal(
                proposal_id=str(uuid.uuid4()),
                market_id=fv.market_id, platform=l2_plat, side=l2_side,
                size_usdc=round(final_size, 2),
                limit_price=round(min(0.999, l2_ask), 4),
                order_type=OrderType.LIMIT, strategy_id=StrategyId.ARB,
        ...
        self.accepted += 1
        logger.info(
            "ARB ACCEPTED market=%s dir=%s size=$%.2f net_edge=%.4f "
            "c1=%.4f c2=%.4f age=%dms",
            fv.market_id, direction, final_size, net_edge,
            c1_frac, c2_frac, signal_age_ms,
        )
        return ArbEvaluation(
            market_id=fv.market_id, evaluated_at=now,
            signal_age_ms=signal_age_ms, arb_signal=fv.arb_signal,
            ...
            direction=direction,
            leg1_cost_frac=c1_frac, leg2_cost_frac=c2_frac,
            net_edge=net_edge, raw_size_usdc=raw_size, final_size_usdc=final_size,
