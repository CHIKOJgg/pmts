...
class KillSwitch:
    ...
    def activate(
        ...
        record = ActivationRecord(
            activated_at=_now_ms(), reason=reason,
            mtm_drawdown=mtm_drawdown, peak_equity=peak_equity,
            current_equity=current_equity, triggering_id=triggering_id,
        )
        self._activations.append(record)
        self._active = True
        logger.critical(
            "KILL SWITCH ACTIVATED reason=%s drawdown=%.2f%% equity=$%.2f peak=$%.2f",
            reason, mtm_drawdown * 100, current_equity, peak_equity,
        )
        return record
    ...
    def reset(self, token: str, operator_id: Optional[str] = None) -> bool:
        ...
        if token != self._token:
            logger.warning(
                "Kill switch reset rejected — wrong token (operator=%s)", operator_id
            )
            return False
        self._resets.append(ResetRecord(
            reset_at=_now_ms(),
    ...
    def audit_trail(self) -> dict:
        return {
            ...
            "activations": [
                {
                    "at": r.activated_at, "reason": r.reason,
                    "drawdown": r.mtm_drawdown, "triggering": r.triggering_id,
                }
                for r in self._activations
            ],
            "resets": [
                {"at": r.reset_at, "operator": r.operator_id}
